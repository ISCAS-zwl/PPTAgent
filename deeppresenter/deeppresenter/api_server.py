    ...
    try:
        ...
        async for yield_msg in loop.run(input_request):
            if isinstance(yield_msg, (str, Path)):
                ...
                # Agent 消息
                role_value = (
                    yield_msg.role.value
                    if hasattr(yield_msg.role, "value")
                    else str(yield_msg.role)
                )
                ...
                if yield_msg.tool_calls:
                    ...
                    for tool_call in yield_msg.tool_calls:
                        ...
                        if tool_name == "generate_slide":
                            ...
                            progress = min(
                                20 + int(70 * slides_generated / total_slides), 90
                            )
                ...
                if role_value == "tool" and yield_msg.text:
                    ...
                    if "num_pages" in text:
                        try:
                            ...
                            # 尝试解析工具返回的字典
                            tool_result = (
                                ast.literal_eval(text) if text.startswith("{") else None
                            )
                            if (
                                tool_result
                                and isinstance(tool_result, dict)
                                and "num_pages" in tool_result
                            ):
                                total_slides = tool_result["num_pages"]
                                logger.info(
                                    f"[SSE] Updated total_slides from inspect_manuscript: {total_slides}"
                                )
                            ...
                            match = re.search(
                                r"['\"]?num_pages['\"]?\s*[:=]\s*(\d+)", text
                            )
                            if match:
                                total_slides = int(match.group(1))
                                logger.info(
                                    f"[SSE] Updated total_slides from regex: {total_slides}"
                                )
